        ...
        {showWelcome && (
          ...
          >
            <motion.div
              ...
              className="text-4xl md:text-5xl lg:text-7xl font-bold tracking-widest uppercase text-slate-900 px-4 text-center"
        ...
        <section className="relative z-20 w-full max-w-7xl mx-auto px-6">
          ...
          >
            {products.map((product) => (
              <motion.a
                key={product.name}
                href={product.comingSoon ? undefined : product.url}
                target={product.comingSoon ? undefined : "_blank"}
                rel={product.comingSoon ? undefined : "noopener noreferrer"}
                variants={item}
                style={{ pointerEvents: product.comingSoon ? 'none' : 'auto' }}
                className={`group relative bg-white rounded-[2.5rem] p-10 md:p-12 flex flex-col items-start transition-all duration-700 ${product.comingSoon ? '' : 'hover:-translate-y-2'} border border-slate-100 overflow-hidden ${product.comingSoon ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'} shadow-sm ${product.border} ${product.shadow}`}
              >

                <div className={`absolute inset-0 bg-gradient-to-br ${product.accent} opacity-0 ${product.comingSoon ? '' : 'group-hover:opacity-100'} transition-opacity duration-700 pointer-events-none rounded-[2.5rem]`} />
                ...
                <div className="space-y-4 z-10 flex-1 w-full">
                  ...
                  <div className={`w-12 h-px bg-slate-200 ${product.comingSoon ? '' : 'group-hover:w-full group-hover:bg-slate-300'} transition-all duration-700 ease-in-out`} />
                ...
                <div className="mt-12 flex items-center justify-between w-full z-10 pt-6">
                  <span className="text-sm font-bold text-slate-400 group-hover:text-slate-900 transition-colors uppercase tracking-[0.2em] flex items-center gap-2">
                    <span className={`w-6 h-px bg-transparent ${product.comingSoon ? '' : 'group-hover:bg-slate-900 origin-left scale-x-0 group-hover:scale-x-100'} transition-all duration-500`} />
                    {product.comingSoon ? "Coming Soon..." : "Learn More"}
                  </span>

                  <div className={`flex items-center justify-center w-14 h-14 rounded-full border border-slate-100 bg-white text-slate-400 ${product.comingSoon ? 'opacity-50' : 'group-hover:bg-slate-900 group-hover:border-slate-900 group-hover:text-white group-hover:rotate-45'} transition-all duration-500 shadow-sm`}>
